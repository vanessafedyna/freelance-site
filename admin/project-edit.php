...
if ($projectId > 0) {
    try {
        ...
        $load = $pdo->prepare(
            'SELECT id, title, slug, category, project_type, project_year, short_description, result_text,
                    thumbnail, link_url, link_label, is_published, display_order
             FROM projects
             WHERE id = :id
             LIMIT 1'
        );
        $load->execute(['id' => $projectId]);
        $project = $load->fetch();
        ...
        if (is_array($project)) {
            ...
            $values = [
                ...
                'category' => (string) ($project['category'] ?? 'site web'),
                'project_type' => (string) ($project['project_type'] ?? 'demo'),
                'project_year' => (string) ($project['project_year'] ?? $currentYear),
                'short_description' => (string) ($project['short_description'] ?? ''),
                'result_text' => (string) ($project['result_text'] ?? ''),
                'thumbnail' => (string) ($project['thumbnail'] ?? ''),
                'link_url' => (string) ($project['link_url'] ?? ''),
                'link_label' => (string) ($project['link_label'] ?? ''),
                'is_published' => (string) ((int) ($project['is_published'] ?? 0)),
                'display_order' => (string) ((int) ($project['display_order'] ?? 0)),
            ];
        }
...
if ($projectFound && (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST')) {
    ...
    if (empty($errors)) {
        try {
            ...
            $checkSlug = $pdo->prepare('SELECT COUNT(*) FROM projects WHERE slug = :slug AND id <> :id');
            $checkSlug->execute([
                'slug' => $values['slug'],
                'id' => $projectId,
            ]);
            ...
            $formError = 'Impossible de mettre à jour le projet pour le moment.';
        }
    }
}
?>
...
<body>
    <main class="container">
        ...
        <?php if ($projectId <= 0): ?>
            <p class="notice">ID de projet invalide.</p>
        <?php elseif ($loadError): ?>
            <p class="notice">Impossible de charger le projet pour le moment.</p>
        <?php elseif (!$projectFound): ?>
            <p class="notice">Projet introuvable.</p>
        <?php else: ?>
            <section class="card">
                <?php if (!empty($errors)): ?>
                ...
                <form method="post" action="project-edit.php?id=<?php echo urlencode((string) $projectId); ?>" enctype="multipart/form-data" novalidate>
                    <div class="grid">
                        ...
                        <div class="field full">
                            ...
                            <?php if ($values['thumbnail'] !== ''): ?>
                                <img src="<?php echo h('../' . $values['thumbnail']); ?>" alt="Miniature actuelle" style="max-height:120px;border-radius:8px;border:1px solid #d8d0c2;margin-bottom:0.4rem;display:block;">
                                <small style="color:#555;margin-bottom:0.3rem;display:block;">Image actuelle — sélectionne un nouveau fichier pour la remplacer.</small>
                            <?php endif; ?>
                            <input type="hidden" name="thumbnail_current" value="<?php echo h($values['thumbnail']); ?>">
